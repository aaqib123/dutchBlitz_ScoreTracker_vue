...
<template>
  <section class="flex flex-col flex-1 mb-20">
    <div class="rounds flex flex-col flex-1 overflow-y-auto gap-5">
      ...
      <RoundCard v-for="(round, index) in store.rounds" :key="round.id" :round-number="index + 1" :round="round"
        :show-delete-button="store.rounds.length - 1 === index" @delete-round="store.deleteRound(round.id)" />
